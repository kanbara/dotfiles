...
def render_templates(template_dir, files, context, postflight):
...
def update_terms(theme_name):
   pts = os.listdir('/dev/pts')
   for each_pts in pts:
      if each_pts.isdigit():
         subprocess.call('echo "`~/.config/base16-shell/{0}.sh`" > /dev/pts/{1}'.format(theme_name, each_pts), shell=True)
...
def reload(theme_name):
    """Reload the given theme."""
...
def generate(color_source, config_file, template_dir, theme_name):
    ...
    # add theme name to config for fish shell colours
    config["variables"]["themeName"] = theme_name;
    ...
    render_templates(template_dir, files, context, postflight)
...
if __name__ == '__main__':
    ...
    action = args[0]
    if action not in ('list', 'activate', 'generate', 'current', 'delete', 'reload', 'update_terms'):
        panic('Unknown action "%s"' % action)

    if action not in ('list', 'current', 'reload') and len(args) == 1:
      ...
      current = os.path.basename(os.path.realpath(
         os.path.join(THEMER_ROOT, 'current')))
      reload(current)
      sys.exit(0);
